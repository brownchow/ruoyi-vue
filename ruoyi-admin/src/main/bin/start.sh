...
# 使用说明：
#   1. 将打好的包解压到目标目录
#   2. 修改 config 目录下的配置文件（特别是数据库连接等配置）
#   3. 执行 ./bin/start.sh 启动应用
# =============================================================================
...
# 启动应用
# --loader.path: 指定外部依赖目录（用于加载lib下的jar）
# --spring.config.location: 指定外部配置文件目录（会替换默认的classpath配置）
# -jar: 运行fat jar
# nohup ... > /dev/null 2>&1 &: 后台启动，不输出到终端
nohup java $JVM_OPTS -Dloader.path="$LIB_DIR" --spring.config.location="file:$CONFIG_DIR/" -jar "$LIB_DIR/$APP_NAME.jar" > /dev/null 2>&1 &
